...
vector pgstat(vector observed_counts, vector background_counts, vector background_error, vector expected_model_counts, int[] idx_background_zero, int[] idx_background_nonzero) {
  ...
  log_likes[idx_background_nonzero] = (
    -square(b[idx_background_nonzero] - background_counts[idx_background_nonzero]) ./ (2 * s2[idx_background_nonzero])
  ...
  for (n in 1:num_elements(idx_background_zero)) { //what happens if observed_counts is zero?
    if (observed_counts[idx_background_zero[n]] > 0) {
      log_likes[idx_background_zero[n]] = lmultiply(
          observed_counts[idx_background_zero[n]], expected_model_counts[idx_background_zero[n]]
        ) - factorial_term[idx_background_zero[n]];
      } else {
      log_likes[idx_background_zero[n]] = - factorial_term[idx_background_zero[n]];
    }
  }
