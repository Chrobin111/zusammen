...
parameters {
  ...
  vector<lower=0>[N_grbs] int_scatter_sq;
...
transformed parameters {
  ...
  vector<lower=0>[N_grbs] int_scatter = sqrt(int_scatter_sq);
...
model {
  ...
  int_scatter_sq ~ normal(0,2.5);
  for (n in 1:N_intervals){
    log_energy_flux[n] ~ normal(log_Nrest[grb_id[n]] - (1.099 + 2 * log10(dl[n])) + gamma[grb_id[n]] * (log10(1 + z[n]) + log_epeak[n] - 2), int_scatter[grb_id[n]]);
  }
