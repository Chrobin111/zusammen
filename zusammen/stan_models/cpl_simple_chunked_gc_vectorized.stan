...
transformed parameters {
  ...
  vector<lower=50>[N_intervals] log_Nrest_vec;
  vector<lower=0>[N_intervals] gamma_vec;
  vector<lower=0>[N_intervals] int_scatter_vec;

  for(n in 1:N_intervals){
    log_Nrest_vec[n] = log_Nrest[grb_id[n]];
    gamma_vec[n] = gamma[grb_id[n]];
    int_scatter_vec[n] = int_scatter[grb_id[n]];
  }
...
model {
  ...
  log_energy_flux ~ normal(
    log_Nrest_vec - (1.099 + 2 * log10(dl)) + gamma_vec .* (log10((1 + z) .* epeak) - 2),
    int_scatter_vec
  );
